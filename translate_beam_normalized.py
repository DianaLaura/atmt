...
def get_args():
    ...
    parser.add_argument('--alpha', default=0.5, type=float, help='parameter that affects how much sentence length is taken into account for length normalization')

    return parser.parse_args()

def length_normalization(decoder_out):
    alpha = args.alpha
    decoder_out= ((5+np.absolute(decoder_out))**alpha)/((5+1)**alpha)
...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        with torch.no_grad():
            ...
            decoder_out = length_normalization(decoder_out) #applies length normalization
        ...
        for _ in range(args.max_len-1):
            ...
            # see __QUESTION 1
            decoder_out = length_normalization(decoder_out) #length normalization function
            log_probs, next_candidates = torch.topk(torch.log(torch.softmax(length_normalization(decoder_out), dim=2)), args.beam_size+1, dim=-1)
