...
def get_args():
    ...
    parser.add_argument('--threshold-src', default=10, type=int,
                        help='map words appearing less than threshold times to unknown')
    parser.add_argument('--num-words-src', default=-1, type=int, help='number of source words to retain')
    parser.add_argument('--threshold-tgt', default=10, type=int,
                        help='map words appearing less than threshold times to unknown')
...
def get_stats(vocab): #source: Sennrich 2016, minimal example on page 4
    pairs = collections.defaultdict(int)
    for word in range(0,vocab.__len__()):
        symbols = vocab.__getitem__(word).split()
        for i in range(len(symbols)-1):
            pairs[symbols[i],symbols[i+1]] += freq 
    return pairs

def merge_vocab(pair, v_in): #source: Sennrich 2016, minimal example on page 4
    v_out = {}
